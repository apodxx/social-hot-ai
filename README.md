...
| `qq` | **QQ 开放平台 Bot（官方 API），已实现但未经真实验证** —— 需要真实凭据 |

⚠️ **`qq` 通道必须明确标注为未验证**：它按公开 API 形状实现（`/app/getAppAccessToken`、
`Authorization: QQBot <token>`、`/v2/users/{openid}/messages`），但项目没有凭据可跑通。
`GET /api/notification/status` 会返回 `"verified": false` 并附说明，不会假装健康。
字符上限也取得保守（800），因为公开文档的精确上限随消息类型变化。
